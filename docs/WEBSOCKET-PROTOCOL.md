...
| `SELL_CARD` | carte de sa main à vendre (1 vente/tour, phase Main ; **aucun Eddie immédiat** — la carte est révélée puis posée face cachée et prête en Eddies Area). **Mini-Feature 10C** : les cartes de type `unit` et `legend` sont refusées (`ILLEGAL_ACTION`, « Les Unités et les Légendes ne peuvent pas être vendues ») ; `program`, `gear`… restent vendables | — | `SellCardCommand` |
