...
- [x] **R5.7 (Mini-Feature 10C, 2026-09-17)** **Restriction de vente par Type de Carte** : les cartes de type `UNIT` et `LEGEND` ne peuvent PAS être vendues ; tous les autres types (`PROGRAM`, `GEAR`…) restent vendables, toujours dans la limite stricte d'1 vente par tour. Refus `GameRuleException` (→ code `ILLEGAL_ACTION` sur le WebSocket) : « Les Unités et les Légendes ne peuvent pas être vendues », sans mutation ni consommation du quota — *Source: brief Mini-Feature 10C (arbitrage utilisateur) + § MAIN PHASE « Sell any card in your hand **with a sell tag** » (les Units/Legends n'en portent pas)* — *Impl: `SellCardCommand.validate` + `SellCardCommand.isSellableType`, miroir frontend `isSellableCardType` (bouton « Vendre » désactivé) et `MockGameServer.sellCard`* — *Tests: `testR_SellCard_ProgramOrGear_Success`, `testR_SellCard_Unit_Rejected`, `testR_SellCard_Legend_Rejected`*

**Implémentation attendue (Mini-Feature 3 — « Vente = Création de ressource ») :** `SellCardCommand.execute` = révélation (`CARD_REVEALED` + événement `EFFECT_RESOLVED`) → `Player.moveToZone(card, EDDIES_AREA)` → `card.setFaceDown(true)` + `card.setExhausted(false)` → `player.setHasSoldThisTurn(true)`. **Aucun `player.addEddy()`** ; le gain d'Eddie passe exclusivement par `SpendEddiesCommand` (R6.1). Limite `GameConstants.SALES_PER_TURN = 1` vérifiée dans `validate`, refus journalisé `ILLEGAL`. **Mini-Feature 10C :** `validate` vérifie aussi le TYPE de la carte trouvée en main — `UNIT`/`LEGEND` ⇒ refus « Les Unités et les Légendes ne peuvent pas être vendues » (garde placée après la limite d'1 vente/tour et la recherche en main, avant toute mutation).
...
- Mini-Feature 10C (2026-09-17) : ✅ 3 tests `SellCardCommandTest` (`testR_SellCard_ProgramOrGear_Success`, `testR_SellCard_Unit_Rejected`, `testR_SellCard_Legend_Rejected`) — restriction de vente par Type de Carte (R5.7) : `UNIT`/`LEGEND` refusées (motif « Les Unités et les Légendes ne peuvent pas être vendues », relayé `ILLEGAL_ACTION`), `PROGRAM`/`GEAR` acceptées dans la limite d'1 vente par tour ; les fixtures de vente des suites existantes (`GameIntegrationTest`, `GameCommandTest`, `SpendResourceCommandTest`, `GameServiceTest`) sont migrées vers des types vendables
